...
describe('hand', () => {
    ...
    describe('#classHand', () => {
        it('should update score for the hand', () => {
            expect(straightFlush.score).toBe(9);
            expect(fourKind.score).toBe(8);
            expect(fullHouse.score).toBe(7);
            expect(straight.score).toBe(6);
            expect(flush.score).toBe(5);
            expect(threeKind.score).toBe(4);
            expect(twoPair.score).toBe(3);
            expect(onePair.score).toBe(2);
            expect(highCard.score).toBe(1);
        });
    });


    describe('#setWinner', () => {
        it('should update the hand to have winner == true', () => {
            expect(straightFlush.setWinner()).toBe(true);
            expect(straightFlush.winner).toBe(true);
        });
    });


    describe('#sortCards', () => {
        it('should sort cards to be highest to lowest', () => {
            expect(straightFlush.sortCards()[0]).toBe(6);
            expect(straightFlush.sortCards()[straightFlush.count() - 1]).toBe(2);
        });
    });


    describe('#toString', () => {
        it('should return cards in hand as a string', () => {
            expect(straightFlush.toString()).toBe('6C 5C 4C 3C 2C');
        });
    });
});
